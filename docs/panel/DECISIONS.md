...
### D-002-a · 추가 요구 — 1.0 이후는 **패치 설치파일**로 업그레이드 (발주자 지시 2026-07-30)

발주자: "이렇게 1.0 버전이 맞으면 그다음 업그레이드는 패치용 설치파일 만들어서 패치시킬까 해."

이 요구는 #6 의 설치 레이아웃을 **처음부터** 규정한다. 나중에 얹을 수 없다 —
1.0 이 잘못된 레이아웃으로 배포되면 패치 자체가 불가능해진다. 그래서 지금 확정한다.

**설치 레이아웃 3분할 (패치 가능성의 전제)**

| 영역 | 위치 | 패치가 건드리는가 | 이유 |
|---|---|---|---|
| 앱 | `<설치폴더>\app\` (server·web·shared·java\out) | **교체 대상** | 버전마다 바뀐다 |
| 런타임 | `<설치폴더>\runtime\` (내장 JRE·node) | 거의 안 건드림 | 수십 MB. 패치 용량의 대부분이라 분리해야 패치가 작아진다 |
| 데이터 | `%LOCALAPPDATA%\OracleTuner\` (config·data·logs·DB) | **절대 안 건드림** | 사용자 자산. 패치가 이걸 덮으면 접속정보·튜닝이력이 날아간다 |

**패치 패키지 구성**
- `manifest.json` — 버전 + **파일별 SHA-256 목록**. 패처가 현재 설치본과 대조해
  **바뀐 파일만** 교체한다. (`tools/build-portable.js` 가 릴리스 때 함께 생성해야 한다.)
- `minVersion` 가드 — 예: 1.0.x 에만 적용 가능. 안 맞으면 거부하고 전체 설치판을 안내한다.
- 롤백: 교체 전 대상 파일을 `<설치폴더>\.rollback\<버전>\` 로 옮겨 두고, 실패 시 되돌린다.
- 패처도 본 인스톨러와 같은 방식(PowerShell + IExpress)으로 만든다. 도구를 늘리지 않는다.

**#4(SQLite)에 걸리는 제약** — 그래서 `server/paths.js` 에 DB 경로 반환 함수를 **한 곳**만 두게 했다.
#6 에서 그 함수 하나만 `%LOCALAPPDATA%` 로 바꾸면 패치 안전성이 확보된다.
또 `meta.schema_version` 을 기록해야 패치가 스키마 마이그레이션 여부를 판단할 수 있다.

**Java 클래스 배포 형태 재검토 필요**: 현재 `java/out/*.class` 를 그대로 쓰는데,
패치 단위로는 **jar 1개**가 훨씬 유리하다(파일 수백 개 대조 대신 1개 교체).
#6 착수 시 `java/build.js` 가 jar 를 만들도록 바꿀지 판단한다.
