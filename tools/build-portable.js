...
function writeLaunchers(stage, nodeExe, withJre) {
  ...
  const bat = [
    ...
    'echo.',
    '',
    // %* — 인자를 그대로 넘긴다. `OracleTuner.bat --port 7071` 로 한 번만 다른 포트를
    // 쓰는 길을 열어둔다(포트 충돌 안내문이 이 사용법을 가리킨다).
...
포트에 대하여
-------------
기본 포트는 7070 입니다. 그 포트를 이미 다른 프로그램(설치판 Oracle Tuner 등)이
쓰고 있으면 **자동으로 비어 있는 다음 포트**(7071, 7080, 8070, 8080, 9070 순)로
띄웁니다. 실제 주소는 트레이 아이콘 툴팁과 logs\\server.log 에 나옵니다.
[열기] 메뉴는 언제나 실제 포트로 엽니다.

포트를 고정하고 싶으면 config\\settings.json 의 server.port 를 바꾸세요
(처음 실행하면 이 파일이 기본값으로 자동 생성됩니다):

  {
    "server": { "port": 7071 }
  }

직접 지정한 포트가 사용 중이면 자동으로 바꾸지 않고 오류로 알립니다.
한 번만 다른 포트로 띄우려면: OracleTuner.bat --port 7071

설치판과 포터블을 함께 써도 됩니다. 서로 다른 폴더에 있으면 트레이·서버·데이터가
모두 따로 돌아갑니다(트레이 아이콘도 각각 하나씩 뜹니다).
